...
const hasUseServerDirective = require('./has-use-server');
  ...
  create(context) {
    return {
      Program(node) {
        ...
        const topLevelUseServer = node.body.some(
          (statement) =>
        ...
        if (!hasRuntimeEdge) {
          context.report({
            ...
            fix(fixer) {
              ...
              // Short-circuit if 'use client'; or top-level 'use server'; directive is present
              if (hasUseClientDirective(body)) {
                return null;
              }

              const topLevelUseServerFix = body.some(
                (statement) =>
                  statement.type === 'ExpressionStatement' &&
                  statement.expression.type === 'Literal' &&
                  statement.expression.value === 'use server'
